    ...
    private void parseCommand(String commandString, boolean reversed) {
        ...
        for (String command : commandList) {
            ...
            switch (commandParts[0]) {
                ...
                // Outtake(time in seconds) - sequential - outtakes the Target
                case "O": {
                ...
                // Intake(time in seconds) - parallel - intakes the Target
                case "I": {
                ...
                // TurnToTarget() - sequential - turn to the nearest Target using vision
                case "TTC": {
                    if (speed == 0) {
                        switch (mode) {
                            case "S":
                                addSequential(new DrivetrainTurnToTarget());
                                break;
                            case "P":
                                addParallel(new DrivetrainTurnToTarget());
                                break;
                            default:
                                addSequential(new DrivetrainTurnToTarget());
                                break;
                        ...
                        switch (mode) {
                            case "S":
                                addSequential(new DrivetrainTurnToTarget(speed));
                                break;
                            case "P":
                                addParallel(new DrivetrainTurnToTarget(speed));
                                break;
                            default:
                                addSequential(new DrivetrainTurnToTarget(speed));
                                break;
